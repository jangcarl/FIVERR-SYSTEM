...
class Offer extends Database
{
    ...
    public function createOffer($user_id, $description, $proposal_id)
    {
        ...
        $result = $this->executeQuery($checkSql, [$user_id, $proposal_id]);

        if ($result && $result[0]['cnt'] > 0) {
            throw new Exception("You have already submitted an offer for this proposal.");
        }


        // Insert if no duplicate
        $sql = "INSERT INTO offers (user_id, description, proposal_id) 
            VALUES (?, ?, ?)";
    ...
    public function checkSubmittedOffer($user_id, $proposal_id)
    {
